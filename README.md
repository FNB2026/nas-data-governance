...
> **Beta 状态提示**：当前版本 0.5.0-beta.1 处于公开测试阶段。核心功能已可日常使用，但尚未完成首次正式签名发布。请在使用前阅读 [用户指南](docs/user-guide/NDG-用户指南.md) 中的安全说明。

## 产品界面

> 截图将在首次正式签名发布前补充。当前可参阅 [用户指南](docs/user-guide/NDG-用户指南.md) 中的界面说明。
...
1. 从 [GitHub Releases](https://github.com/FNB2026/nas-data-governance/releases) 下载 `NDG-<版本号>-macos.dmg`
...
NDG 不主动连接互联网服务：当前不包含遥测、云上传、远程 API 或外部 AI 调用。

- 不上传你的文件、文件名、路径、哈希值或元数据
- 不收集使用统计、崩溃报告或性能数据
- 哈希计算、重复检测、治理分析和决策逻辑由 Mac 本地进程完成
- 当用户选择已挂载的 NAS 目录时，文件访问由 macOS 通过 SMB、NFS 等已配置的文件系统连接完成
- 项目数据库保存在 Mac 应用支持目录，隔离文件保存在用户指定的隔离根目录（可能位于本地磁盘、外接存储或 NAS）
