...
本仓库依据《NAS 数据整理与归档概念手册》建立。当前实现安全索引、目录语境、去重计划与受限的安全执行内核：只读扫描、元数据采集、分层哈希、完全重复报告、可解释的目录角色、仅供审阅的计划，以及隔离执行器。CLI 目前没有 `execute` 子命令；任何文件写入只能通过显式批准的计划和配置了源根目录的执行器发生。
