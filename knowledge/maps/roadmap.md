...
- 已完成 L3：行业资料学习（corpus）。`LearnFromCorpus()` 读取受信目录 `var/corpus/` 的 TXT/MD/DOCX 资料提取文本（PDF 延后），CJK 2-4 字 n-gram 分词，跳过 builtin 已覆盖术语；行业敏感词（诊断/处方等）路由到 SensitiveCandidates 生成 RoleSensitive 草案；`GenerateCorpusDrafts()` 持久化 status=draft 规则，priority≤60（K-008），保留已审批规则不被覆盖；CLI `learn --source=corpus --corpus-dir [--apply] [--out]` 子命令，默认只读干跑。L4（决策反馈学习）仍待实施。
