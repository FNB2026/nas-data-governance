...
- 已完成 L2：本地统计学习（learning 包）。`Learn()` 遍历 SQLite 索引统计目录名频次与共现，跳过敏感目录（K-009）与 builtin 已覆盖术语；`GenerateDrafts()` 生成 status=draft 规则草案并持久化，强制 priority≤60（K-008），保留已审批规则不被覆盖；co-occurrence 平局确定性打破（内置权威优先→术语字典序）；CLI `learn --db [--apply] [--out]` 子命令，默认只读干跑。L3（行业资料学习）/L4（决策反馈学习）仍待实施。
